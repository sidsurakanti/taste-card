...
class SpotifyOAuth {
  ...
  /**
   * @param {string} code - The authorization code returned after spotify user authorization
  ...
  /**
   * @returns {Promise<SpotifyClientCredsToken>} a token that can be used to access the Spotify API
  ...
  /**
   * @param {string} refresh_token - The refresh token returned by the Authorization Code Flow
   * @returns {Promise<SpotifyAuthToken>} token - A new token that can be used to access the Spotify API
  ...
  /**
   * @returns {string}  The base64 encoded client credentials
   */
  ...
  /**
   * @param {RequestCookies} cookies - The cookies object from the request
   * @param {SpotifyAuthToken} token - The token to add to the cookies
   */
  static addTokenToCookies(
...
class SpotifyAPI {
  ...
  public async getArtistWithName(name: string): Promise<SpotifyArtist> {
    ...
    if (!res.ok) {
      ...
      throw new Error("ERROR FETCHING SPOTIFY ARTIST BY NAME", data);
    }
